...
public class AnnotationPrinter extends CasConsumer_ImplBase implements CasObjectProcessor {
  ...
  public synchronized void processCas(CAS aCAS) throws ResourceProcessException {
    ...
    Iterator annotationIter = (Iterator) jcas.getIndexRepository().getIndex("geneindex").iterator();
    while (annotationIter.hasNext()) {
      Sentence annot = (Sentence) annotationIter.next();
      String aText = annot.getCoveredText();
      aText = aText.replace('\n', ' ');
      aText = aText.replace('\r', ' ');
      try {
        fileWriter.write(annot.getSentenceId()+"|"+annot.getBegin()+" "+annot.getEnd() +"|"+ annot.getTextstring() + "\n");
        fileWriter.flush();
      } catch (IOException e) {
        throw new ResourceProcessException(e);
      }
